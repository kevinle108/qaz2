...
function updateCandidateSelection(statement, voteCount) {
	...
	if (candidate.candidateCode == 'writein') {
		statement = addWriteInName(statement, candidate.candidateName)
	}
	return statement
}
...
function addWriteInName(statement, writeInName) {
	//AddElement|elementType|parentElement|title[|x|y|fontCode|fontSize|textString]||
	let strs = statement.split('|')
	let newStatement = ''
	for (let i=0; i<9; i++) {
		newStatement += strs[i] + '|'
	...
	let writeInX = strs[4] 
	let writeInY = strs[10] - 6
	let size = strs[7] - 1
	newStatement += writeInX + '|' + writeInY + '|' + strs[6] + '|' + size + '|' + ' ' + writeInName  
	for (let i=9; i<strs.length; i++) {
		newStatement += '|' + strs[i] 
	}	
	return newStatement
}
